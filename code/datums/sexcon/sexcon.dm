...
/datum/sex_controller/proc/cum_into(oral = FALSE)
	...
	SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "sex", /datum/mood_event/sex)
	if(oral)
...
/datum/sex_controller/proc/ejaculate()
	...
	if(user.getorganslot(ORGAN_SLOT_VAGINA))
		add_cum_floor(get_turf(user), TRUE)
	if(user.getorganslot(ORGAN_SLOT_PENIS))
		add_cum_floor(get_turf(user), FALSE)
	after_ejaculation()
...
/datum/sex_controller/proc/after_ejaculation()
	...
	SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "climax", /datum/mood_event/climax)

/datum/sex_controller/proc/after_intimate_climax()
	if(user == target)
		return
	// TODO Chances for inspirations?
...
/datum/sex_controller/proc/set_arousal(amount)
	...
	update_blueballs()
	update_erect_state()
	update_horny_feedback()

/datum/sex_controller/proc/update_horny_feedback()
	if(last_horny_msg + (5 MINUTES) > world.time)
		return
	if(arousal < 50)
		return
	var/static/list/messages = list("You feel a little frisky...", "You really need some right now!", "You could go down on someone...")
	var/picked = pick(messages)
	to_chat(user, SPAN_LOVE(picked))
...
/datum/sex_controller/proc/adjust_arousal(amount, max)
	var/total = arousal + amount
	if(max && total > max)
		total = max
	set_arousal(total)
...
/datum/sex_controller/proc/update_blueballs()
	if(arousal >= BLUEBALLS_GAIN_THRESHOLD)
		SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "blueb", /datum/mood_event/blueb)
	else if (arousal <= BLUEBALLS_LOOSE_THRESHOLD)
		SEND_SIGNAL(user, COMSIG_CLEAR_MOOD_EVENT, "blueb", /datum/mood_event/blueb)
...
/datum/sex_controller/proc/do_after_check()
	if(desire_stop)
		return FALSE
	if(!current_action)
		return FALSE
...
/datum/sex_controller/proc/sex_action_loop()
	...
	while(TRUE)
		if(!do_after(user, (action.do_time / get_speed_multiplier()), target = target, extra_checks = CALLBACK(src, PROC_REF(do_after_check))))
			break
